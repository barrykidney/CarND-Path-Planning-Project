    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); ++i) {
            ...
            if (i_d < (2+4*lane+2) && i_d > (2+4*lane-2)) {
              ...
              if ((check_car_s > car_s) && (check_car_s - car_s) < 30) {
                ...
                if (lane > 0 && !left_lane_blocked) {
                  lane--;
                } else if (lane <2 && !right_lane_blocked) {
                  lane++;
                }
              }
            }
          ...
          if (too_close) {
            ref_vel -= .224; // make the brake amt porportional to the dist that the blocking car is ahead
          } else if (ref_vel < 49.5) { // make the acc amt porportional to the dist that the road is clear ahead
            ref_vel += .224;
          }
